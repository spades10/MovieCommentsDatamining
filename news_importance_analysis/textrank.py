...
class Docrank:
    ...
    def timeline(self, nodes_rank):
        f_story = open('timelines.txt', 'w+', encoding='utf-8')
        f_important = open('important_doc.txt', 'w+', encoding='utf-8')
        date_dict = {}
        timelines = {}
        ...
        for date, doc_dict in date_dict.items():
            f = open(os.path.join(self.storypath, str(date)), 'w+', errors='ignore')
...
print(result)
f = open('../result/result_news/important_doc.txt', 'w', encoding='utf-8')
for ss in result:
    f.write(ss[0] + '\t' + str(ss[1]) + '\r')
